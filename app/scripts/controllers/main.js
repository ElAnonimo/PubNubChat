  ...
  .controller('MainCtrl', ['$scope', '$rootScope', '$location', 'Pubnub', function($scope, $rootScope, $location, Pubnub) {
		...
		$scope.createChannel = function() {
			...
			Pubnub.ngGrant({
				...
				write: true,
				callback: function() {
					return console.log(channel + ' channel all set.', arguments);
				}
			});
			...
			Pubnub.ngGrant({
				...
				write: false,
				callback: function() {
					return console.log(channel + ' channel presense all set.', arguments);
				}
			});
		...
		// Get a reasonable historical backlog of messaged to populate the channels list
		Pubnub.ngHistory({
			channel: $scope.controlChannel,
			count: 500
		});
		
		// Put a new user to the 'Waiting Room' channel
